...
namespace Devdog.Rucksack.Items.Editor
{
    public abstract class ObjectEditorBase<T> : UnityEditor.Editor
    {
        ...
        /// </summary>
        /// <returns></returns>
        protected bool DoesInheritFromParent<TValueType>(string propertyName, TValueType currentChainValue, UnityItemDefinition startObj)
        {
            ...
            while (parent != null)
            {
                ...
                if (Equals(startValue, GetDefault(type)) && Equals(currentChainValue, null) == false)
                {
                    return true;
                }
