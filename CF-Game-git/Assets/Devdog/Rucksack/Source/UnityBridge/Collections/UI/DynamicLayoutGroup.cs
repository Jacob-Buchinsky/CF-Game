...
{
    [AddComponentMenu(RucksackConstants.AddPath + "Layout/Dynamic Layout Group")]
    public class DynamicLayoutGroup : LayoutGroup
    {
        ...
        private CollectionSlotUIBase<IItemInstance>[] _slots = new CollectionSlotUIBase<IItemInstance>[0];
...
//            SetLayoutVertical(true);
        ...
        {
            var l = new List<CollectionSlotUIBase<IItemInstance>>(rectChildren.Count);
            foreach (var child in rectChildren)
            {
                var c = child.GetComponent<CollectionSlotUIBase<IItemInstance>>();
        ...
        private void SetCellsAlongAxis(int axis, bool forceRebuild = false)
        {
            ...
            else
            {
                ...
                {
                    var c = rectChildren[i].GetComponent<CollectionSlotUIBase<IItemInstance>>();
                    if (c != null)
                    {
                        if (Equals(_slots[i].current, c.current) == false)
                        {
                            shouldRebuild = true;
                            break;
                        }
...
//                cellCountX = Mathf.Max(1, Mathf.FloorToInt((width - padding.horizontal + spacing.x + 0.001f) / (cellSize.x + spacing.x)));
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                int layoutCols = 1;
                int layoutRows = 1;
                if (_slots[i].current != null)
                {
                    layoutCols = _slots[i].current.itemDefinition.layoutShape.convexX;
                    layoutRows = _slots[i].current.itemDefinition.layoutShape.convexY;
                }
        ...
        private void CheckBlocks()
        {
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                var item = _slots[i].current;
                if (item == null)
                {
                    continue;
                }

                for (int j = 0; j < item.itemDefinition.layoutShape.convexX; j++)
                {
                    if (j > 0)
                    {
                    ...
                    for (int k = 0; k < item.itemDefinition.layoutShape.convexY; k++)
                    {
                        if (k > 0 || (j > 0 && k == 0))
                        {
