...
//                    }
        ...
        /// </summary>
        public virtual void IndexSlotsAndMountPoints()
        {
            var col = collection as EquipmentCollection<IEquippableItemInstance>;
            if (col == null)
            {
                logger.Warning($"Character equipment collection is not of type {typeof(EquipmentCollection<IEquippableItemInstance>).Name}. Can not itemGuid existing slots...", this);
                return;
            }

            if (col.characterOwner == null)
            {
                logger.Warning($"UnityEquipmentCollection does not have an assigned character. Can not itemGuid slots", this);
                return;
            }
