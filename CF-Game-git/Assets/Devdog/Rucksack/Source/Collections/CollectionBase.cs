...
namespace Devdog.Rucksack.Collections
{
        ...
        where TElementType : class, IEquatable<TElementType>, IStackable, IIdentifiable, ICloneable
    {
        ...
        public event EventHandler<CollectionRemoveResult> OnRemovedItem;
        ...
        public Result<CollectionRemoveResult> RemoveBoxed(object item, int amount = 1, CollectionContext context = null)
        {
            if (item is TElementType == false && item != null)
            {
                return new Result<CollectionRemoveResult>(null, Errors.CollectionInvalidItemType);
            }
            
            return Remove((TElementType) item, amount, context);
        }
...
//                throw new ArgumentException($"Given item is not convertible to {typeof(TElementType).Name} - {item} :: {item.GetType().Name}");
        ...
        public Result<CollectionRemoveResult> Remove(TElementType item, int amount = 1)
        {
        ...
        public virtual void Sort(IComparer<TElementType> comparer, CollectionContext context)
        {
            ...
            // NOTE: Keep in mind the size of certain slots < Needs clever packing.
