...
namespace Devdog.Rucksack.Collections
{
    public class LayoutCollection<TElementType> : CollectionBase<LayoutCollectionSlot<TElementType>, TElementType>
        where TElementType : class, IEquatable<TElementType>, IShapeOwner2D, IStackable, IIdentifiable, ICloneable
    {
        ...
        private bool CanSetAABB(int index, TElementType item, int amount, CollectionContext context)
        {
            ...
            if ((index % columnCount) + Offset(item.shape.convexX - 1, 0) >= columnCount ||
                index + Offset(item.shape.convexX - 1, item.shape.convexY - 1) >= slotCount)
            {
                return false;
            }
            ...
            for (int x = 0; x < item.shape.convexX; x++)
            {
                for (int y = 0; y < item.shape.convexY; y++)
                {
                    // We're not occupying this specific slot.
                    if (item.shape.IsBlocking(x, y) == false)
                    {
                        continue;
                    }
