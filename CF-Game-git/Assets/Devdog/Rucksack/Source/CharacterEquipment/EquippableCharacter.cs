...
namespace Devdog.Rucksack.CharacterEquipment
{
        ...
        where TEquippableType : class, TBaseType, IEquatable<TEquippableType>, IEquippable<TEquippableType>, Collections.ICollectionSlotEntry, ICloneable
    {
        ...
        public virtual List<EquippableSlotAmount> GetEquippableSlots(TEquippableType item, int equipAmount)
        {
            ...
            if (collection.GetCanAddAmount(item, equipAmount).result >= equipAmount)
            {
                ...
                for (int i = 0; i < collection.slotCount; i++)
                {
                    ...
                    {
                        added = Math.Min(collection.GetAmount(i) - equipAmount, Math.Min(equipAmount, item.maxStackSize));
        ...
        public virtual Result<bool> Equip(TEquippableType item, int amount = 1)
        {
            ...
            {
                var equipped = DoEquipAt(slot.slot, item, amount, slot.unEquipContents);
        ...
        public virtual Result<bool> EquipAt(int index, TEquippableType item, int amount = 1)
        {
            var canEquip = CanEquip(item, amount);
            if (canEquip.error != null)
            {
        ...
        private Result<bool> DoEquipAt(int index, TEquippableType item, int amount, bool unEquipContents)
        {
        ...
        public virtual Result<bool> UnEquip(TEquippableType item, int amount = 1)
        {
        ...
        {
            return collection.SwapOrMerge(from, collection, to, collection.GetAmount(from));
        }
    }
}